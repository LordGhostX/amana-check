...
| T3  | Provider retention or training on prompts             | Every request hardcodes `zdr: true` and `data_collection: "deny"`. If no compliant endpoint exists the call throws and the request fails closed instead of degrading to a provider that retains prompts. Prompt and completion content is never logged.                                                                      |
...
| T8  | Admin takeover                                        | Passcode login is timing-safe and rate limited to ten attempts per fifteen minutes. It issues an HttpOnly, SameSite=Lax, HMAC-signed session token that expires after eight hours and is Secure in production. Forged and expired tokens are rejected in tests.                                                              |
...
- Stop model spend by setting `DAILY_COST_LIMIT_USD=0` and restarting, or by removing the OpenRouter key.
- Purge the check corpus with `DELETE FROM claims;` if the retention policy changes.
- Rotate `IP_HASH_SECRET` to invalidate rate-limit keys and admin sessions. Rotate `ADMIN_PASSCODE` to revoke operator access.
